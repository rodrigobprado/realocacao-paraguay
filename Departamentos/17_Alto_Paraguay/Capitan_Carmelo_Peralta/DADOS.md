# Avaliacao de Localidade: Capitan Carmelo Peralta, Alto Paraguay

## Pesquisa oficial consolidada

### 1. Geografia e contexto estrategico/militar
- **Coordenadas:** 21°41′S 57°54′W.
- **Topografia:** Relevo plano (geologicamente estável, baixo risco sísmico), margeado pelo Rio Paraguai (fronteira com Brasil). Área de ~4.798 km².
- **Alvos Estratégicos:** Ponte Bioceânica (em fase final de construção - conexão vital Paraguai-Brasil); Eixo da Ruta PY15 (Corredor Bioceânico); Porto fluvial; Proximidade com o Pantanal paraguaio. Localização estratégica absoluta como hub logístico internacional de carga (celulose, grãos e proteína).
- **Fallout:** Ventos predominantes do Norte/Nordeste.

    - Ministerio de Defensa Nacional: https://mdn.gov.py/
    - Portal Geoestadistico INE: https://www.ine.gov.py/portalgeoestad/

### 2. Populacao e infraestrutura
- **População:** 4.402 (Censo 2022 Final). População em rápido crescimento devido ao impacto das obras de infraestrutura. Presença de comunidades indígenas Ayoreo.
- **Densidade:** ~0,92 hab/km² (**Extremamente baixa**, oferecendo isolamento geográfico massivo fora do núcleo urbano).
- **Vias de Acesso:** Transformação radical com o Corredor Bioceânico pavimentado (PY15) e a nova ponte internacional para Porto Murtinho (Brasil). Risco elevado de saturação e controle militar em crises devido ao valor logístico da rota.
- **Serviços:** Infraestrutura urbana e de serviços em fase de expansão acelerada. Dependência residual de centros maiores para saúde de alta complexidade.
- **Custo de Vida:** Em ascensão devido à valorização imobiliária; energia elétrica barata (ANDE ~US$ 0,05/kWh).
- **Preço da Terra:** US$ 300-600/ha (bruta) a US$ 1.200/ha (desenvolvida). Forte especulação em áreas logísticas.

    - Censo 2022 INE: https://www.ine.gov.py/censo2022/
    - Indicadores distritais INE: https://www.ine.gov.py/vt/Indicadores-distritales.php

### Indicadores Sociais

**Fontes:** PNUD 2020, INE Censo 2022, INE EPHC 2023, Ministerio Público 2024  
**Nota:** Valores marcados como *(dept.)* referem-se ao departamento de Alto Paraguay; valores *(dist.)* são específicos deste distrito. Dados marcados *(est.)* são estimativas.

| Indicador | Valor | Âmbito |
|-----------|-------|--------|
| IDH (2020) | 0,619 | dept. |
| Ranking IDH nacional | 18/18 (ou 17/18, disputado com Caazapá) | dept. |
| Esperança de vida | 68,4 anos | dept. |
| Escolaridade média | 5,8 anos | dept. |
| RNB per capita (USD PPA) | 4.900 | dept. |
| Pobreza monetária (%) | 38,7% | dept. |
| Pobreza extrema (%) | estimativa 14,2% | dept. |
| Índice de Gini | estimativa 0,478 | dept. |
| Acesso a água potável (%) | estimativa 34,5% | dept. |
| Acesso a saneamento (%) | estimativa 31,2% | dept. |
| Taxa de homicídios (est., /100k hab) | ~2–4 (estimativa, abaixo da média nacional) | dept. |
| Índice de segurança | alto | dept. |
| População (Censo 2022) | 4.402 | dist. |
| Idade mediana | N/D | dist. |
| Taxa de fecundidade | N/D | dist. |

### 3. Dados Climáticos e Ambientais

**Fonte climática:** NASA POWER Climatology API (período 2001-2020)
**Fonte luminosa:** estimativa_world_atlas

#### Irradiação Solar (kWh/m²/dia)

| Jan | Fev | Mar | Abr | Mai | Jun | Jul | Ago | Set | Out | Nov | Dez | Média |
|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-------|
| 6.74 | 6.20 | 5.50 | 4.48 | 3.35 | 2.88 | 3.21 | 3.96 | 4.59 | 5.43 | 6.43 | 6.81 | **4.96** |

**Inclinação solar recomendada:** 25° N (anual) · 35° N (inverno jun-ago) · 15° N (verão nov-jan)

#### Precipitação (mm/mês)

| Jan | Fev | Mar | Abr | Mai | Jun | Jul | Ago | Set | Out | Nov | Dez | Total/ano |
|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----------|
| 131 | 136 | 131 | 152 | 172 | 90 | 75 | 52 | 92 | 187 | 190 | 166 | **1577 mm** |

#### Poluição Luminosa

| Parâmetro | Valor |
|-----------|-------|
| Escala Bortle | 1 — Céu verdadeiramente escuro |
| Radiância artificial | 0.1 nW/cm²/sr |

### 4. Riscos naturais
- **Hidrologia:** **Risco de Inundação Sazonal.** Localidade margeada pelo Rio Paraguai, vulnerável a cheias extraordinárias. Obras de defesa costeira em andamento. Risco de incêndios florestais em períodos de seca no Chaco.
- **Clima:** Tropical Úmido/Seco. Verões com calor extremo. Déficit hídrico é o principal desafio agrícola.
- **Sismicidade:** Risco baixo; região estável com eventos intraplaca raros e de baixa magnitude (<4.0 Richter).

    - DMH avisos: https://www.meteorologia.gov.py/avisos/
    - SEN: https://sen.gov.py/acciones/

### 5. Recursos e autossuficiencia
- **Energia:** Rede nacional (Itaipu) em fase de reforço para atender ao novo hub logístico.
- **Água:** Rio Paraguai e sistemas de tratamento local.
- **Qualidade do Solo:** Solos aluviais ricos em fósforo; desafios de drenagem e salinidade. Potencial agrícola em expansão (soja, milho, algodão) com manejo adequado.
- **Resources Locais:** Pecuária de corte intensiva e pesca. Elevadíssimo potencial para autossuficiência a nível de proteína animal devido à baixa densidade e vastidão territorial. Localidade estratégica para suprimento de longo prazo se a logística bioceânica estiver operacional.

    - ANDE: https://www.ande.gov.py
    - MOPC: https://mopc.gov.py/

#### Solo (SoilGrids 2.0, média ponderada 0–30 cm)

| Parâmetro | Valor |
|-----------|-------|
| pH (H₂O) | 6.25 |
| Carbono orgânico (SOC) | 14.97 g/kg |
| Argila | 32.23 % |
| Areia | 31.27 % |
| Silte (calc.) | 36.5 % |
| Densidade aparente | 1.31 g/cm³ |
| **Aptidão agrícola** | **Alta** |

Fonte: ISRIC SoilGrids 2.0 via WCS. Coords: -21.6833°, -57.9°. Média ponderada camadas 0-5, 5-15, 15-30 cm.

### 6. Ambiente sociopolitico
- **Segurança:** Zona de fronteira em transformação. Baixa criminalidade urbana violenta tradicional, mas com desafios crescentes ligados ao fluxo internacional de carga. Coesão social desafiada pelo rápido crescimento populacional e migratório.
- **Leis Local:** Município consolidado e foco prioritário de investimentos nacionais. **Restrição de Fronteira (Lei 2532/05):** Estrangeiros de países limítrofes não podem possuir terras em faixa de 50km da fronteira sem decreto de interesse público.

    - TSJE: https://www.tsje.gov.py
    - Dados abertos: https://www.datos.gov.py/

---

## 7. Vulnerabilidades e Mitigacao
- **Exposicao Hidrologica:** Risco de inundação sazonal pelo Rio Paraguai (reforço de defesas costeiras e construção elevada recomendada).
- **Isolamento Logístico:** Dependência de poucas rotas de saída; o Corredor Bioceânico é vital (monitorar tráfego e possíveis bloqueios).
- **Segurança de Fronteira:** Proximidade com o Brasil exige vigilância contra fluxos migratórios descontrolados ou crises transfronteiriças.
- **Autossuficiência Energética:** Baixa diversidade de fontes locais (potencializar microgeração solar para reduzir dependência da rede).

## 6. Pontuacao GSS (avaliacao calibrada - Onda 2)
Notas (0-10):
- A: 5.0 (Hub logístico internacional; alvo estratégico em conflitos)
- B: 7.5 (Baixíssima densidade demográfica compensada pelo rápido crescimento)
- C: 6.5 (Geologia estável no Chaco; vulnerabilidade hídrica sazonal)
- D: 8.0 (Elevado potencial de autossuficiência proteica e hub de suprimentos)
- E: 7.4 (Forte suporte governamental; restrição legal de fronteira para estrangeiros)

Calculo:
- GSS = ((A x 2.5) + (B x 2.0) + (C x 1.5) + (D x 2.0) + (E x 2.0)) / 10
- GSS: 6.8

Classificacao:
- Moderadamente Seguro (Exige preparação e infraestrutura dedicada).

## 8. Dados Consolidados de Fontes Oficiais
- Fonte INE: https://www.ine.gov.py/censo2022/
- Fonte INE indicadores distritais: https://www.ine.gov.py/vt/Indicadores-distritales.php
- Fonte MOPC infraestrutura: https://mopc.gov.py/servicios/estado-de-las-rutas/
- Fonte DMH risco/clima: https://www.meteorologia.gov.py/avisos/
- Fonte SEN eventos/acoes: https://sen.gov.py/acciones/

Sintese aplicada:
- Dados textuais consolidados com base nas fontes oficiais acima; integração de novas infraestruturas logísticas na análise estratégica.

## 9. Indicadores Quantificados
- **Populacao municipal:** 4.402 habitantes (Censo 2022 Final).
- **Densidade demografica:** ~0,92 hab/km² (Extremamente baixa).
- **Custo de Terra Rural:** US$ 300-1.200 por hectare (Chaco/Fronteira).
- **Matriz Energetica:** ~US$ 0,05/kWh (Tarifa ANDE).
- **Exposicao Sismica:** Risco desprezível (<4.0 Richter).

## 10. Analise de Riscos
- **Cenario curto prazo:** Especulação imobiliária agressiva e impacto social das obras da ponte.
- **Cenario medio prazo:** Saturação das rotas logísticas e aumento da presença militar na zona de fronteira.
- **Mitigacao:** Aquisição antecipada de terras (respeitando leis locais), implementação de sistemas de água independentes.

## 12. Analise de Sensibilidade (Onda 2)
- Cenario otimista (A+1, D+1): GSS 7.25
- Cenario conservador (B-1, C-1): GSS 6.45
- Amplitude de sensibilidade: 0.8 ponto(s)
- Leitura: Variabilidade ligada ao sucesso da integração logística e controle de riscos hídricos.

## 11. Redacao Final
### Diagnostico Integrado
Capitán Carmelo Peralta (17_Alto Paraguay) é um hub de fronteira em transição radical. Deixou de ser uma zona isolada para se tornar o epicentro da Rota Bioceânica, o que traz oportunidades massivas de autossuficiência proteica e valorização, mas também riscos estratégicos elevados por ser um nó logístico internacional.

### Por que sim
- Isolamento geográfico por densidade, mas com conectividade moderna de alta qualidade.
- Potencial pecuário e recursos hídricos significativos.
- Suporte estatal prioritário por ser projeto de interesse nacional.

### Por que nao
- Alvo estratégico evidente em cenários de conflito logístico/fronteiriço.
- Risco de inundação sazonal requer obras de proteção caras.
- Restrições de compra de terra para estrangeiros de países limítrofes.

### Pre-condicoes Minimas de Decisao
- Validação jurídica de aquisição de terras (Lei 2532/05).
- Avaliação da cota de inundação no terreno específico.
- Implementação de autonomia energética (Solar) e de água.

### Recomendacao Tecnica
Uso recomendado para perfis que buscam valorização estratégica e autossuficiência em áreas de baixa densidade, cientes do papel geopolítico do hub. Referencia atual de score: GSS 6.8 em 2026-03-05; risco predominante: moderado (hidrológico e estratégico).

## 13. Matriz de Evidencias (E1)
- 1) Delimitacao territorial validada por georreferencia institucional (Fonte: https://www.ine.gov.py/portalgeoestad/; acesso: 2026-03-05).
- 2) Populacao municipal referenciada por Censo 2022 (Fonte: https://www.ine.gov.py/censo2022/; acesso: 2026-03-05).
- 3) Indicadores de infraestrutura e rodovias PY15 (Fonte: https://mopc.gov.py/servicios/estado-de-las-rutas/; acesso: 2026-03-05).
- 4) Riscos climáticos e hidrológicos (Fonte: https://www.meteorologia.gov.py/avisos/; acesso: 2026-03-05).
- 5) Informações de custo de energia e matriz (Fonte: https://www.ande.gov.py; acesso: 2026-03-05).
- 6) Restrições legais de fronteira - Lei 2532/05 (Fonte: https://www.datos.gov.py/; acesso: 2026-03-06).

Regra aplicada: quando serie oficial distrital granular nao esta disponivel publicamente, a lacuna e registrada no dossie sem interromper a cadeia analitica.

### Combustível

**Referência:** PETROPAR / postos locais (2024)
**Tipo de localidade:** Interior

| Combustível | USD/litro | Gs/litro (aprox.) |
|-------------|-----------|-------------------|
| Gasolina 93 oct | 1.18 | 8,732 |
| Gasolina 97 oct (premium) | 1.28 | 9,472 |
| Diesel | 1.10 | 8,140 |

> Preços podem variar ±5% conforme posto e sazonalidade. Chaco e interior remoto apresentam maior variação.

### Cobertura Celular

**Fonte:** CONATEL PY / operadoras (2024)

| Parâmetro | Valor |
|-----------|-------|
| Cobertura 4G população (dept.) | 35% |
| Cobertura 4G área rural | 15% |
| Melhor operadora | Tigo |
| Qualidade rural | limitada |

> Para áreas rurais fora do núcleo urbano, recomenda-se chip Tigo como principal e Personal como backup.

### Internet

**Fonte:** CONATEL / Speedtest Ookla (2024)

| Parâmetro | Valor |
|-----------|-------|
| Velocidade média download | 15 Mbps |
| Domicílios com internet (dept.) | 22% |
| Tecnologia predominante | satélite |
| Opção rural | Starlink disponível (~USD 44/mês) |

### Mercado Imobiliário e Terra Rural

**Fonte:** INDERT / Clasificados.com.py (2024)

| Tipo | Referência |
|------|-----------|
| Terra agrícola alta prod. (USD/ha) | 600 |
| Imóvel urbano (USD/m²) | 400 |
| Aluguel 2 quartos (USD/mês) | 160 |

> Valores de referência departamental. Localidades menores podem ter preços 20–40% abaixo da capital departamental.
### Saúde

**Fonte:** MSPBS / IPS Paraguay (2024-2026), consolidação departamental e proxy local

| Serviço | Disponibilidade |
|---------|----------------|
| USF / Posto de Saúde | sim |
| Hospital Regional | não |
| IPS (seguro social) | não |
| Farmácia | sim |
| Distância ao hospital de referência | ~641 km (Fuerte Olimpo) |

**Principais estabelecimentos:** USF local; referencia hospitalar em Fuerte Olimpo

**Observação para imigrantes:** Atendimento primario local ou em raio curto; casos de maior complexidade seguem para Fuerte Olimpo. Cobertura privada continua recomendada para especialidades e urgências de maior complexidade.
